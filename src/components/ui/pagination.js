...
export default function Pagination({ currentPage, totalPages, onPageChange }) {
  ...
  return (
    <div className='flex justify-center items-center gap-1 mt-3 text-sm'>
      <button
        className='px-2.5 py-1.5 rounded-md border border-gray-300 text-gray-600 hover:bg-gray-100 transition disabled:opacity-50 disabled:cursor-not-allowed'
      ...
      {pages.map((page) => (
        <button
          key={`page-${page}`}
          className={`w-8 h-8 flex items-center justify-center rounded-md border border-gray-300 transition-all duration-200 text-sm ${
            page === currentPage
